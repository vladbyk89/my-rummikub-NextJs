...
export const game = createSlice({
	...
	reducers: {
		...
		moveFromBoardToHand: (state, action) => {
			...
			if (
				!state.activePlayer.hand.startHand.some(
		...
		endActivePlayerTurn: (state) => {
			...
			if (!playerMadeAMove(state)) {
				state.activePlayer.hand.endHand =
					state.activePlayer.hand.startHand;
			...
			state.activePlayer = getNextPlayer(
				state.players,
				activePlayerIndex
			);
